...
function initBlogChrome() { 
  ...
  if (window.applyBoardBannerUI) window.applyBoardBannerUI();
} 
...
function getBoardPath() {
  ...
  if (!boardId) {
    ...
    if (path.endsWith('sketches.html')) boardId = 'sketches';
    else if (path.endsWith('galery.html') || path.endsWith('gallery.html')) boardId = 'galery';
    else if (path.endsWith('random.html')) boardId = 'random';
    else if (path.endsWith('index.html') || path.endsWith('/')) boardId = 'home';
    else boardId = 'blog';
  }
  return (boardId === 'blog') ? 'blog/posts' : 
         (boardId === 'sketches') ? 'gallery/sketches' : 
         (boardId === 'galery' || boardId === 'gallery') ? 'gallery/pictures' : 
...
function checkLockdown() {
  ...
  const curBoardId = getCurBoardId();
  const isBoardLockdown = window._globalSettings && window._globalSettings.lockedBoards && (window._globalSettings.lockedBoards[curBoardId] === true || (curBoardId === 'gallery' && window._globalSettings.lockedBoards.galery === true));
  const isAtPages = window.location.pathname.includes('/assets/pages/');
  const isAtPageNotFound = window.location.pathname.endsWith('pagenotfound.html');

  // 1. Board Lockdown check
  if (isBoardLockdown && !isAdmin && !isAtPageNotFound && curBoardId !== 'home') {
    window.location.href = isAtPages ? 'pagenotfound.html' : 'assets/pages/pagenotfound.html';
    return;
  }

  // 2. Total Lockdown check
  ...
  if (isLockdown && !isAdmin) {
    if (!overlay) {
      ...
      overlay.className = 'crazy-flashing';
      
      // Inject crazy-flashing style dynamically if not already injected
      if (!document.getElementById('crazyFlashStyle')) {
        const style = document.createElement('style');
        style.id = 'crazyFlashStyle';
        style.textContent = `
          ...
          .crazy-flashing {
            animation: crazyFlash 0.05s infinite !important;
          }
        `;
    ...
    if (isUrlAdmin) {
      overlay.innerHTML = `
        <button class="form-btn" id="lockdownLoginBtn" style="padding: 12px 24px; font-size: 14px; cursor: pointer; background: #800000; color: #fff; border: 2px solid #000; font-weight: bold; box-shadow: 4px 4px 0px #000;">
          Login with Google
        </button>
      `;
      ...
      overlay.innerHTML = ''; // Absolutely no messages or buttons when not in ?admin=true
    }
...
function renderBoardList() {
  ...
  // blog/sketches/home are static, hard-coded boards, not real dynamic boards.
  // Their settings are also saved under admin/boards/<id>/settings (so features can be
  // toggled), but that must never make them show up as a second, duplicate board here.
  const STATIC_BOARD_IDS = ['blog', 'sketches', 'home'];
  const boardEntries = Object.entries(boards).filter(([id]) => !STATIC_BOARD_IDS.includes(id));
...
function applyBoardBannerUI() {
  const boardId = getCurBoardId();
  const boards = window._boards || {};
  let boardMeta = boards[boardId] || {};
  
  if (boardId === 'blog' || boardId === 'sketches') {
    if (boards[boardId]) {
      boardMeta = boards[boardId];
    }
  }
  ...
  let parent = bannerImg.parentNode;
  if (parent && !parent.classList.contains('banner-wrapper')) {
    const wrapper = document.createElement('div');
    wrapper.className = 'banner-wrapper';
    wrapper.style.position = 'relative';
    wrapper.style.display = 'inline-block';
    
    parent.replaceChild(wrapper, bannerImg);
    wrapper.appendChild(bannerImg);
    
    const blText = document.createElement('span');
    blText.id = 'bannerBottomLeftText';
    blText.style.cssText = "position: absolute; bottom: 5px; left: 10px; font-family: 'Times New Roman', serif; font-size: 16px; color: black; -webkit-text-stroke: 1px white; font-weight: bold; text-shadow: none; pointer-events: none;";
    wrapper.appendChild(blText);
    
    const brText = document.createElement('span');
    brText.id = 'bannerBottomRightText';
    brText.style.cssText = "position: absolute; bottom: 5px; right: 10px; font-family: 'Times New Roman', serif; font-size: 16px; color: black; -webkit-text-stroke: 1px white; font-weight: bold; text-shadow: none; pointer-events: none;";
    wrapper.appendChild(brText);
  }
  
  const blTextEl = document.getElementById('bannerBottomLeftText');
  const brTextEl = document.getElementById('bannerBottomRightText');
  
  const isHome = boardId === 'home';
  if (isHome) {
    if (globalSettings.banner) {
      bannerImg.src = globalSettings.banner;
    }
    if (blTextEl) blTextEl.style.display = 'none';
    if (brTextEl) brTextEl.style.display = 'none';
  } else {
    const customBanner = boardMeta.banner || '';
    if (customBanner) {
      bannerImg.src = customBanner;
    } else {
      if (boardId === 'blog') bannerImg.src = location.pathname.includes('/assets/pages/') ? '../images/BLOGIMGBAN.jpg' : 'assets/images/BLOGIMGBAN.jpg';
      else if (boardId === 'sketches') bannerImg.src = location.pathname.includes('/assets/pages/') ? '../images/SKETCHESIMGBAN.jpg' : 'assets/images/SKETCHESIMGBAN.jpg';
      else bannerImg.src = location.pathname.includes('/assets/pages/') ? '../images/RANDOMIMGBAN.jpg' : 'assets/images/RANDOMIMGBAN.jpg';
    }
    
    const subtitleEl = document.querySelector('.subtitle');
    if (subtitleEl) {
      const customDesc = boardMeta.description || '';
      if (customDesc) {
        subtitleEl.textContent = '/' + boardId + ' - ' + customDesc;
      } else {
        if (boardId === 'blog') subtitleEl.textContent = '/blog - post dump, thoughts etc...';
        else if (boardId === 'sketches') subtitleEl.textContent = '/sketches - art archive';
        else subtitleEl.textContent = '/' + boardId + ' - custom board';
      }
    }
    
    if (blTextEl) {
      blTextEl.style.display = '';
      blTextEl.textContent = globalSettings.defaultAuthor || 'yuri.neet';
    }
    if (brTextEl) {
      brTextEl.style.display = '';
      brTextEl.textContent = boardMeta.bottomRightText || ('/*' + boardId + '*/');
    }

    if (window._isAdmin) {
      let editBtn = document.getElementById('editBannerBtn');
      if (!editBtn) {
        editBtn = document.createElement('button');
        editBtn.id = 'editBannerBtn';
        editBtn.className = 'form-btn';
        editBtn.textContent = 'Edit Banner & Description';
        editBtn.style.cssText = 'display: block; margin: 5px auto; padding: 4px 10px; cursor: pointer; font-weight: bold; background: #800000; color: #fff; border: 1px solid #000;';
        editBtn.onclick = () => openEditBannerModal();
        
        const bannerWrapper = document.querySelector('.banner-wrapper') || bannerImg.parentNode;
        bannerWrapper.parentNode.insertBefore(editBtn, bannerWrapper);
      }
    }
  }
  
  const faviconUrl = boardMeta.favicon || globalSettings.favicon || '';
  if (faviconUrl) {
...
window.applyBoardBannerUI = applyBoardBannerUI;
...
function openEditBannerModal() {
  ...
  let wrap = document.getElementById('editBannerDialog');
  if (!wrap) {
    wrap = document.createElement('div');
    wrap.id = 'editBannerDialog';
    wrap.className = 'settings-backdrop';
    wrap.addEventListener('click', event => {
      if (event.target === wrap) closeEditBannerModal();
    });
    document.body.appendChild(wrap);
  }

  const currentBanner = boardMeta.banner || '';
  const currentDesc = boardMeta.description || '';
  const currentBRText = boardMeta.bottomRightText || ('/*' + boardId + '*/');

  wrap.innerHTML = `
    <div class="settings-dialog" role="dialog" aria-modal="true" style="max-width: 400px; background: var(--color-bg-alt, #fff); border: 1px solid #000; box-shadow: 2px 2px 8px rgba(0,0,0,0.5);">
      <div class="settings-head" style="background: #800000; color: #fff; padding: 6px 10px; display: flex; justify-content: space-between; align-items: center; font-weight: bold;">
        <span>Edit Banner & Info</span>
        <button type="button" class="form-btn" onclick="closeEditBannerModal()" style="color: #fff; background: none; border: none; cursor: pointer; font-weight: bold;">x</button>
      </div>
      <div class="settings-body" style="padding: 15px; background: var(--color-bg-alt, #fff); color: var(--color-fg-main, #000); display: block; text-align: left;">
        <div style="margin-bottom: 10px;">
          <label style="font-weight: bold; display: block; margin-bottom: 4px;">Banner Image URL</label>
          <input type="text" id="editBannerUrl" value="\${esc(currentBanner)}" placeholder="URL..." style="width: 100%; box-sizing: border-box; margin-bottom: 5px;">
          <button class="form-btn" onclick="document.getElementById('editBannerFileInput').click()">Upload File</button>
          <input type="file" id="editBannerFileInput" accept="image/*" onchange="handleEditBannerUpload(event)" style="display:none;">
          <span id="editBannerUploadStatus" style="font-size: 10px; color: #888; margin-left: 5px;"></span>
        </div>
        <div style="margin-bottom: 10px;">
          <label style="font-weight: bold; display: block; margin-bottom: 4px;">Board Description</label>
          <input type="text" id="editBannerDesc" value="\${esc(currentDesc)}" placeholder="Description..." style="width: 100%; box-sizing: border-box;">
        </div>
        <div style="margin-bottom: 15px;">
          <label style="font-weight: bold; display: block; margin-bottom: 4px;">Bottom Right Text</label>
          <input type="text" id="editBannerBRText" value="\${esc(currentBRText)}" placeholder="/*\${esc(boardId)}*/" style="width: 100%; box-sizing: border-box;">
        </div>
        <div style="text-align: right; border-top: 1px dashed #ccc; padding-top: 10px;">
          <button type="button" class="form-btn" onclick="saveEditBannerData()" style="padding: 4px 12px; cursor: pointer; font-weight: bold;">Save</button>
          <button type="button" class="form-btn" onclick="closeEditBannerModal()" style="padding: 4px 12px; cursor: pointer; margin-left: 5px;">Cancel</button>
        </div>
      </div>
    </div>
  `;
...
function closeEditBannerModal() {
  document.getElementById('editBannerDialog')?.classList.remove('is-open');
}
window.closeEditBannerModal = closeEditBannerModal;

async function handleEditBannerUpload(event) {
  ...
  try {
    ...
    document.getElementById('editBannerUrl').value = data.secure_url;
    if (statusEl) statusEl.textContent = 'ready!';
  } catch (err) {
    if (statusEl) statusEl.textContent = 'upload error: ' + err.message;
  }
}
window.handleEditBannerUpload = handleEditBannerUpload;

async function saveEditBannerData() {
  const boardId = getCurBoardId();
  const banner = document.getElementById('editBannerUrl').value.trim();
  const description = document.getElementById('editBannerDesc').value.trim();
  const bottomRightText = document.getElementById('editBannerBRText').value.trim();

  try {
    await window._set(window._ref(window._db, \`admin/boards/\${boardId}/banner\`), banner || null);
    await window._set(window._ref(window._db, \`admin/boards/\${boardId}/description\`), description || null);
    await window._set(window._ref(window._db, \`admin/boards/\${boardId}/bottomRightText\`), bottomRightText || null);
    
    // Update local cache
    window._boards = window._boards || {};
    window._boards[boardId] = {
      ...(window._boards[boardId] || {}),
      banner,
      description,
      bottomRightText
    ...
    alert('Banner details updated!');
    closeEditBannerModal();
    applyBoardBannerUI();
  } catch (e) {
    alert('Error saving banner details: ' + e.message);
  }
}
window.saveEditBannerData = saveEditBannerData;

let _curBoardBannerMedia = null;
async function handleCurBoardBannerSelect(event) {
  const file = event.target.files[0];
  if (!file) return;
  const statusEl = document.getElementById('curBoardBannerStatus');
  ...
  try {
    ...
    const data = await res.json();
    if (!data.secure_url) throw new Error('upload failed');
    _curBoardBannerMedia = data.secure_url;
    if (statusEl) statusEl.textContent = 'banner uploaded!';
    document.getElementById('curBoardBannerInput').value = data.secure_url;
  } catch (err) {
    if (statusEl) statusEl.textContent = 'upload error: ' + err.message;
  }
}
window.handleCurBoardBannerSelect = handleCurBoardBannerSelect;

let _curBoardFaviconMedia = null;
async function handleCurBoardFaviconSelect(event) {
  const file = event.target.files[0];
  if (!file) return;
  const statusEl = document.getElementById('curBoardFaviconStatus');
  if (statusEl) statusEl.textContent = 'uploading favicon...';
  try {
    const fd = new FormData();
    fd.append('file', file);
    fd.append('upload_preset', 'unsigned_upload');
    ...
    const data = await res.json();
    if (!data.secure_url) throw new Error('upload failed');
    _curBoardFaviconMedia = data.secure_url;
    if (statusEl) statusEl.textContent = 'favicon uploaded!';
    document.getElementById('curBoardFaviconInput').value = data.secure_url;
  } catch (err) {
    if (statusEl) statusEl.textContent = 'upload error: ' + err.message;
  }
}
window.handleCurBoardFaviconSelect = handleCurBoardFaviconSelect;
...
function renderBlogEditorMedia() {
  ...
  if (pv) {
    ...
    pv.innerHTML = _blogMedia.map((item, index) => {
      const url = esc(item.url || '');
      let thumbHTML = '';
      if (item.type?.startsWith('video')) {
        thumbHTML = `<video src="${url}" style="width:50px; height:50px; object-fit:cover; border:1px solid #aaa;"></video>`;
      } else if (item.type?.startsWith('audio')) {
        thumbHTML = `<div style="width:50px; height:50px; border:1px solid #aaa; display:flex; align-items:center; justify-content:center; font-size:9px; background:#f0f0f0;">Audio</div>`;
      } else {
        thumbHTML = `<img src="${url}" style="width:50px; height:50px; object-fit:cover; border:1px solid #aaa;">`;
      }
      return `
        <div class="uploaded-file-item" style="display:flex; align-items:center; gap:6px; background:#fafafa; border:1px solid #ccc; padding:4px; margin-bottom:4px; width:100%; box-sizing:border-box;">
          ${thumbHTML}
          <div style="flex-grow:1; font-size:10px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; text-align:left;">
            <strong>${esc(item.info?.name || 'file')}</strong> (${esc(item.info?.sizeKB || '?')} KB)
          </div>
          <button type="button" class="form-btn" style="padding:1px 4px; font-size:10px; cursor:pointer;" onclick="removeBlogEditorFile(${index})">remove</button>
        </div>
      `;
    }).join('');
  }
...
function removeBlogEditorFile(index) {
  _blogMedia.splice(index, 1);
  renderBlogEditorMedia();
}
window.removeBlogEditorFile = removeBlogEditorFile;
...
function updateAdminBoardList() {
  ...
  const boards = window._boards || {};
  const STATIC_BOARD_IDS = ['blog', 'sketches', 'home'];
  container.innerHTML = Object.entries(boards)
    .filter(([id]) => !STATIC_BOARD_IDS.includes(id))
    .map(([id, b]) => `
...
function injectAdminUI() { 
  ...
  const lockedBoards = currentSettings.lockedBoards || {};
  renderBoardList();
        ...
        <div id="adminPanel" style="display: block;"> 
          ...
          <div id="admin-tab-editor">
            ...
            <tr> 
              ...
              <td><input type="text" id="postAuthor" style="width:45%;box-sizing:border-box;background:#f0f0f0;color:#555;" readonly> <span style="font-size:10px;color:var(--color-fg-muted);">locked to global author</span></td> 
            </tr> 
            <tr> 
              <td class="form-label">Title</td> 
              <td><input type="text" id="postTitle" placeholder="Title..." style="width:100%;box-sizing:border-box;"></td> 
            </tr> 
            <tr id="postMoodRow"> 
              <td class="form-label">Mood</td> 
              <td> 
                <input type="text" id="postMoodText" placeholder="mood name..." style="width:50%;"> 
                <select id="postMoodEmote" style="width:35%;" onchange="syncMoodIcon()">${moodOptions}</select> 
          ...
          <div id="admin-tab-batch" style="display:none;">
            ...
            <div style="padding:10px;">
              <div style="margin-bottom: 12px; display: flex; gap: 8px;">
                <button class="form-btn" onclick="batchMarkSpoiler()">Mark Spoiler</button>
                <button class="form-btn" onclick="batchTogglePrivate()">Toggle Private</button>
                <button class="form-btn" onclick="batchDelete()">Delete Selected</button>
              </div>
              <hr>
          ...
          <div id="admin-tab-global" style="display:none;">
            ...
            <table class="form-table">
              <tr>
                ...
                <td>
                  <select id="admGlobalTheme">
                    <option value="futaba" selected>Futaba classic</option>
                  </select>
                </td>
              </tr>
              <tr>
              ...
              <tr>
                <td class="form-label" style="font-weight:bold;">Enable Mood</td>
                <td>
                  <input type="checkbox" id="admGlobalMoodEnabled" ${currentSettings.moodEnabled !== false ? 'checked' : ''}>
                  <span style="font-size:10px; color:var(--color-fg-muted);">Allow/Disallow the Mood field in the editor</span>
                </td>
              </tr>
              <tr>
                ...
                <td>
                  ...
                  <span style="font-size:10px; color:var(--color-danger);">Locks every page behind a flashing screen</span>
                </td>
              </tr>
              <tr>
                <td class="form-label" style="font-weight:bold; color:var(--color-danger);">Lock Boards</td>
                <td>
                  <label style="display:block;"><input type="checkbox" id="admGlobalLockBlog" ${lockedBoards.blog ? 'checked' : ''}> Lock /blog</label>
                  <label style="display:block;"><input type="checkbox" id="admGlobalLockSketches" ${lockedBoards.sketches ? 'checked' : ''}> Lock /sketches</label>
                  <label style="display:block;"><input type="checkbox" id="admGlobalLockGalery" ${lockedBoards.galery ? 'checked' : ''}> Lock /galery</label>
                  <label style="display:block;"><input type="checkbox" id="admGlobalLockRandom" ${lockedBoards.random ? 'checked' : ''}> Lock /random</label>
                </td>
              </tr>
              <tr>
  ...
  if (authorInput && !authorInput.value) authorInput.value = window._globalSettings?.defaultAuthor || 'yuri.neet';
  bindBlogEditorControls(); 
  if (window.updateEditorFields) window.updateEditorFields();
} 
window.injectAdminUI = injectAdminUI; 

async function batchMarkSpoiler() {
  const checked = Array.from(document.querySelectorAll('.batch-check:checked')).map(cb => cb.dataset.id);
  if (!checked.length) return;
  if (!confirm(`Mark ${checked.length} selected posts as spoiler?`)) return;
  try {
    const path = getBoardPath();
    for (const id of checked) {
      await window._set(window._ref(window._db, path + '/' + id + '/spoilerImg'), true);
    }
    alert('Marked as spoiler!');
    filterPosts();
  } catch (e) { alert('Error: ' + e.message); }
}
window.batchMarkSpoiler = batchMarkSpoiler;

async function batchTogglePrivate() {
  const checked = Array.from(document.querySelectorAll('.batch-check:checked')).map(cb => cb.dataset.id);
  if (!checked.length) return;
  if (!confirm(`Toggle private status for ${checked.length} selected posts?`)) return;
  try {
    const path = getBoardPath();
    for (const id of checked) {
      const p = findPostById(id);
    ...
    alert('Toggled private status!');
    filterPosts();
  } catch (e) { alert('Error: ' + e.message); }
}
window.batchTogglePrivate = batchTogglePrivate;
...
async function saveCurBoardSettings() {
  ...
  try {
    ...
    // Update the local cache immediately so the panel reflects exactly what was saved,
    // instead of resetting to defaults while waiting for the realtime listener to catch up.
    window._boards = window._boards || {};
    window._boards[boardId] = { ...(window._boards[boardId] || {}), settings };
