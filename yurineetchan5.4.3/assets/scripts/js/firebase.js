...
onValue(ref(db, 'admin/settings'), (snapshot) => {
  ...
  if (window.checkBoardLockdown) {
    window.checkBoardLockdown();
  }
});
...
onValue(ref(db, 'admin/boards'), (snapshot) => {
  ...
  if (window.checkBoardLockdown) window.checkBoardLockdown();
});
...
onValue(ref(db, 'gallery/pictures'), (snap) => {
  window._boardPostCounts['galery'] = Object.keys(snap.val() || {}).length;
  if (window.renderBoardList) window.renderBoardList();
});
onValue(ref(db, 'random/posts'), (snap) => {
  window._boardPostCounts['random'] = Object.keys(snap.val() || {}).length;
  if (window.renderBoardList) window.renderBoardList();
});
