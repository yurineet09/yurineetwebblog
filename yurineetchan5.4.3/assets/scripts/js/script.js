...
const STATIC_BOARDS = [
  { id: 'blog', title: 'blog', file: 'blog.html', path: 'blog/posts', defaultBanner: 'BLOGIMGBAN.jpg', description: 'post dump, thoughts, diary, etc...' },
  { id: 'sketches', title: 'sketches', file: 'sketches.html', path: 'gallery/sketches', defaultBanner: 'SKETCHESIMGBAN.jpg', description: 'art archive' },
  { id: 'galery', title: 'galery', file: 'galery.html', path: 'gallery/pictures', defaultBanner: 'RANDOMIMGBAN.jpg', description: 'media archive' },
  { id: 'random', title: 'random', file: 'random.html', path: 'random/posts', defaultBanner: 'RANDOMIMGBAN.jpg', description: 'any other post' }
];
const STATIC_BOARD_IDS = STATIC_BOARDS.map(board => board.id);
function normalizeBoardId(id) {
  if (id === 'gallery') return 'galery';
  return id || '';
}
function staticBoardById(id) {
  const normalized = normalizeBoardId(id);
  return STATIC_BOARDS.find(board => board.id === normalized) || null;
}
function boardPageUrl(board) {
  return location.pathname.includes('/assets/pages/') ? board.file : `assets/pages/${board.file}`;
}
function boardDefaultBannerUrl(board) {
  const prefix = location.pathname.includes('/assets/pages/') ? '../images/' : 'assets/images/';
  return prefix + board.defaultBanner;
}
...
function buildPostPayloadHTML(payload) {
  ...
  if (isSimple || mediaItems.length === 0) {
    ...
    mediaHTML += `<div class="multi-file-panel">
      <div class="multi-file-head">${mediaItems.length} files: ${mediaItems.map(m => esc(m.info?.name || 'file')).join(', ')}</div>`;
...
function buildMoodHTML(payload) {
  if (window._globalSettings?.moodEnabled === false) return '';
...
function buildPosterMetaHTML(payload = {}) {
  const author = String(window._globalSettings?.defaultAuthor || payload.author || 'yuri.neet').trim();
  return `
...
function getBoardPath() {
  ...
  let boardId = normalizeBoardId(urlParams.get('board'));
  if (!boardId) {
    if (window.location.pathname.endsWith('sketches.html')) boardId = 'sketches';
    else if (window.location.pathname.endsWith('galery.html') || window.location.pathname.endsWith('gallery.html')) boardId = 'galery';
    else if (window.location.pathname.endsWith('random.html')) boardId = 'random';
    else if (window.location.pathname.endsWith('index.html') || window.location.pathname.endsWith('/')) boardId = 'home';
    else boardId = 'blog';
  }
  const board = staticBoardById(boardId);
  if (board) return board.path;
  return boardId === 'home' ? 'home/posts' : 'blog/posts';
}
...
function openLoginModal() {
  ...
  wrap.innerHTML = `
    <div class="settings-dialog" role="dialog" aria-modal="true" style="max-width: 320px; background: var(--color-bg-alt, #fff); border: 1px solid #000;">
...
const getCountText = (id) => {
  ...
  const fixedHtml = STATIC_BOARDS
  .map(b => ` [<a href="${boardPageUrl(b)}">/${esc(b.title)}/</a>${getCountText(b.id)}] `)
.join(' / ');

  containers.forEach(c => {
c.innerHTML = fixedHtml;  });

  const footerHtml = fixedHtml;
  ...
  footerContainers.forEach(c => {
    if (c.classList.contains('boards-body')) {
     const listHtml =
  STATIC_BOARDS
    .map(b => `<a href="${boardPageUrl(b)}">/${esc(b.title)}/</a>`)
    .join('');
...
function applyBoardBannerUI() {
  ...
  if (boardId === 'blog' || boardId === 'sketches' || boardId === 'gallery' || boardId === 'random') {
    if (boards[boardId]) {
  ...
  // Render Edit button above banner if logged in as admin
  const isAdmin = window._isAdmin;
  ...
  if (isAdmin && !isHome) {
    if (!editBtn) {
      ...
      editBtn.style.cssText = 'display:block; margin: 6px auto; cursor:pointer; font-weight:bold;';
      editBtn.textContent = '[Edit Banner & Subtitle]';
      editBtn.onclick = (e) => {
        e.preventDefault();
        openBannerEditPopup();
      };
      bannerImg.parentNode.insertBefore(editBtn, bannerImg);
    }
  } else {
  ...
  if (parent && !parent.classList.contains('banner-wrapper')) {
    ...
    blText.style.cssText = "position: absolute; bottom: 5px; left: 10px; font-family: 'Times New Roman', serif; font-size: 26px; color: black; -webkit-text-stroke: 1px white; font-weight: bold; text-shadow: none; pointer-events: none;";
    wrapper.appendChild(blText);
    
    const brText = document.createElement('span');
    brText.id = 'bannerBottomRightText';
    brText.style.cssText = "position: absolute; bottom: 5px; right: 10px; font-family: 'Times New Roman', serif; font-size: 26px; color: black; -webkit-text-stroke: 1px white; font-weight: bold; text-shadow: none; pointer-events: none;";
  ...
  if (isHome) {
    ...
    if (customBanner) {
      ...
      else bannerImg.src = location.pathname.includes('/assets/pages/') ? '../images/RANDOMIMGBAN2.jpg' : 'assets/images/RANDOMIMGBAN2.jpg';
    }
    ...
    if (subtitleEl) {
      ...
      if (customDesc) {
        ...
        else if (boardId === 'gallery') subtitleEl.textContent = '/gallery - media archive';
        else if (boardId === 'random') subtitleEl.textContent = '/random - random discussion';
...
function openBannerEditPopup() {
  const boardId = getCurBoardId();
  const boards = window._boards || {};
  const boardMeta = boards[boardId] || {};
  const banner = boardMeta.banner || '';
  const description = boardMeta.description || '';
  const bottomRightText = boardMeta.bottomRightText || `/*${boardId}/`;

  let wrap = document.getElementById('bannerEditDialog');
  if (!wrap) {
    wrap = document.createElement('div');
    wrap.id = 'bannerEditDialog';
    wrap.className = 'settings-backdrop';
    wrap.addEventListener('click', event => {
      if (event.target === wrap) closeBannerEditPopup();
    });
    document.body.appendChild(wrap);
  }

  wrap.innerHTML = `
    <div class="settings-dialog" role="dialog" aria-modal="true" style="max-width: 440px;">
      <div class="settings-head" style="background: #800000; color: #fff; padding: 6px 10px; display: flex; justify-content: space-between; align-items: center; font-weight: bold;">
        <span>Edit Board /${esc(boardId)}/</span>
        <button type="button" class="close-btn" onclick="closeBannerEditPopup()" style="color: #fff !important;">x</button>
      </div>
      <div class="settings-body" style="padding: 15px; display: flex; flex-direction: column; gap: 10px; grid-template-columns: 1fr;">
        
        <div style="text-align: left;">
          <label style="display:block; font-weight:bold; margin-bottom:4px;">Banner Image URL</label>
          <input type="text" id="editBoardBannerUrl" value="${esc(banner)}" style="width: 100%; box-sizing: border-box; background: #fff; color: #000; border: 1px solid #aaa; padding: 4px;">
        </div>

        <div style="text-align: left;">
          <label style="display:block; font-weight:bold; margin-bottom:4px;">Upload Banner File</label>
          <input type="file" id="editBoardBannerFile" accept="image/*" onchange="handleBoardBannerUpload(event)" style="width:100%;">
          <div id="editBoardBannerStatus" style="font-size:10px; color:#888; margin-top: 4px;"></div>
        </div>

        <div style="text-align: left;">
          <label style="display:block; font-weight:bold; margin-bottom:4px;">Board Description</label>
          <input type="text" id="editBoardDescription" value="${esc(description)}" style="width: 100%; box-sizing: border-box; background: #fff; color: #000; border: 1px solid #aaa; padding: 4px;">
        </div>

        <div style="text-align: left;">
          <label style="display:block; font-weight:bold; margin-bottom:4px;">Bottom Right Text</label>
          <input type="text" id="editBoardBottomRightText" value="${esc(bottomRightText)}" style="width: 100%; box-sizing: border-box; background: #fff; color: #000; border: 1px solid #aaa; padding: 4px;">
        </div>

        <div style="display:flex; justify-content:flex-end; gap:8px; margin-top:10px;">
          <button class="form-btn" onclick="saveBoardBannerEdits()">Save</button>
          <button class="form-btn" onclick="closeBannerEditPopup()">Cancel</button>
        </div>
...
window.openBannerEditPopup = openBannerEditPopup;

function closeBannerEditPopup() {
  document.getElementById('bannerEditDialog')?.classList.remove('is-open');
}
window.closeBannerEditPopup = closeBannerEditPopup;

async function handleBoardBannerUpload(event) {
  const file = event.target.files[0];
  if (!file) return;
  const statusEl = document.getElementById('editBoardBannerStatus');
  ...
  try {
    ...
    document.getElementById('editBoardBannerUrl').value = data.secure_url;
    if (statusEl) statusEl.textContent = 'uploaded successfully!';
  } catch (err) {
    if (statusEl) statusEl.textContent = 'upload error: ' + err.message;
  }
}
window.handleBoardBannerUpload = handleBoardBannerUpload;

async function saveBoardBannerEdits() {
  const boardId = getCurBoardId();
  const banner = document.getElementById('editBoardBannerUrl').value.trim();
  const description = document.getElementById('editBoardDescription').value.trim();
  const bottomRightText = document.getElementById('editBoardBottomRightText').value.trim();

  try {
    await window._set(window._ref(window._db, `admin/boards/${boardId}`), {
      title: boardId,
      banner,
      description,
      bottomRightText
    });
    closeBannerEditPopup();
    alert('Board banner & settings saved!');
  } catch (e) {
    alert('Error saving board settings: ' + e.message);
  }
}
window.saveBoardBannerEdits = saveBoardBannerEdits;
...
function resetPostEditor() {
  ...
  window._replyingToPostId = null;
  window._editingReply = null;
  const author = document.getElementById('postAuthor'); if (author) author.value = (window._globalSettings?.defaultAuthor || 'yuri.neet');
  ...
  const privateBox = document.getElementById('edPrivate'); if (privateBox) privateBox.checked = false;
...
function beginEditPost(id) {
  ...
  window._replyingToPostId = null;
  window._editingReply = null;
  const author = document.getElementById('postAuthor'); if (author) author.value = window._globalSettings?.defaultAuthor || payload.author || 'yuri.neet';
  ...
  const privateBox = document.getElementById('edPrivate'); if (privateBox) privateBox.checked = !!p.private;
...
async function submitPost() { 
  ...
  const author = (window._globalSettings?.defaultAuthor || 'yuri.neet').trim();
  ...
  const payload = postPayloadFrom({
    author: author || null,
    title: title || null,
    moodText: window._globalSettings?.moodEnabled === false ? null : (moodText || null),
    moodEmote: window._globalSettings?.moodEnabled === false ? null : (moodEmote || null),
...
async function saveGlobalSettings() {
  ...
  const lockedBoards = {};
  STATIC_BOARDS.forEach(board => {
    lockedBoards[board.id] = document.getElementById(`admBoardLock_${board.id}`)?.checked || false;
  });
  const moodEnabled = document.getElementById('admGlobalMoodEnabled')?.checked !== false;
...
function injectAdminUI() { 
  ...
  // Non-admins (and anyone not signed in) must never see the admin panel box.
  // The only way to reach admin features is [LOG IN] -> "Login with Google" modal.
  if (!window._isAdmin) {
    c.innerHTML = '';
    renderBoardList();
    return;
            ...
            <table class="form-table">
              ...
              <tr>
                ...
                <td>
                  ...
                  <span style="font-size:10px; color:var(--color-danger);">Locks every page behind a black screen</span>
                </td>
              </tr>
              <tr>
...
function updateEditorFields() {
  ...
  const moodRow = document.getElementById('postMoodText')?.closest('tr');
  if (moodRow) moodRow.style.display = window._globalSettings?.moodEnabled === false ? 'none' : '';
