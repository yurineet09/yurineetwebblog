...
async function saveGlobalSettings() {
  ...
  const commentLimit = parseInt(document.getElementById('admGlobalCommentLimit')?.value || '70', 10) || 70;
        ...
        <div id="adminPanel" style="display: ${window._isAdmin ? 'block' : 'none'};"> 
          ...
          <div id="admin-tab-global" style="display:none;">
            ...
            <table class="form-table">
              ...
              <tr>
                <td class="form-label">Comment Limit</td>
                <td>
                  <input type="number" id="admGlobalCommentLimit" value="${currentSettings.commentLimit || 70}" min="1" max="1000" style="width: 100px;">
                  <span style="font-size:10px; color:var(--color-fg-muted);">Maximum allowed characters per comment</span>
                </td>
              </tr>
              <tr>
...
async function voteSketch(id, type) {
  if (!id) return;
  if (type === 'up') {
    const upvotedSketches = JSON.parse(sessionStorage.getItem('upvoted_sketches') || '[]');
    if (upvotedSketches.includes(id)) {
      alert("You have already upvoted this sketch in this session!");
      return;
    }
    upvotedSketches.push(id);
    sessionStorage.setItem('upvoted_sketches', JSON.stringify(upvotedSketches));
  }
...
function initTopSketchesSidebar() {
  ...
  if (window._globalSettings && window._globalSettings.showTopSketches === false) {
    sidebar.style.display = 'none';
  } else if (window._globalSettings && window._globalSettings.showTopSketches !== false) {
...
function renderCommentsTree(postId, comments) {
  ...
  function buildCommentHTML(comment, depth = 0) {
    ...
    const num = commentNumMap[id];
    
    const replyPrefix = comment.replyTo ? '<span class="quote" style="color:var(--color-fg-quote); font-weight: normal;">>>No.' + commentNumMap[comment.replyTo] + '</span><br>' : '';
    const delBtn = window._isAdmin ? '<a onclick="deleteComment(\'' + esc(postId) + '\', \'' + esc(id) + '\')" style="cursor: pointer; text-decoration: underline; color: #cc0000; margin-left: 8px;">[delete]</a>' : '';

    let html = `
      <div class="anonymous-comment" id="comment-\${esc(id)}" style="margin-left: \${indent}px; padding-left: 0; margin-top: 6px; margin-bottom: 6px;">
        <div style="font-size: 10px; color: #666; display: flex; align-items: center; gap: 6px;">
          <span style="font-weight: bold; color: #111;">\${esc(comment.author || 'Anonymous')}</span>
          <span>\${dateStr}</span>
          <span>No.\${num}</span>
          <a onclick="toggleCommentReplyForm('\\\${esc(postId)}', '\\\${esc(id)}', \${num})" style="cursor: pointer; text-decoration: underline; color: #800000;">[reply]</a>
          \${delBtn}
        </div>
        <div style="font-size: 11px; margin-top: 2px; white-space: pre-wrap; word-break: break-all;">\${replyPrefix}\${escapedText}</div>
        <div id="reply-form-\${esc(id)}" style="display: none; margin-top: 4px; padding-left: 10px;">
          ...
          <div style="margin-top: 2px; text-align: right;">
            ...
            <button class="form-btn" style="padding: 1px 4px; font-size: 10px;" onclick="toggleCommentReplyForm('\\\${esc(postId)}', '\\\${esc(id)}', \${num})">Cancel</button>
          </div>
        </div>
      </div>
    `;
...
function toggleCommentReplyForm(postId, commentId, commentNum) {
...
async function deleteComment(postId, commentId) {
  if (!window._isAdmin || !postId || !commentId) return;
  if (!confirm('delete comment?')) return;
  try {
    const path = getBoardPath() + '/' + postId + '/anonymous_comments/' + commentId;
...
async function submitAnonymousComment(postId, replyToCommentId) {
  ...
  const maxLimit = window._globalSettings?.commentLimit !== undefined ? parseInt(window._globalSettings.commentLimit, 10) : 70;
  if (text.length > maxLimit) {
    alert("Comment is too long! Max " + maxLimit + " characters.");
    return;
  }
  ...
  const comment = {
    text: text,
    ts: now,
    author: window._isAdmin ? 'yuri.neet' : 'Anonymous'
  };
  if (replyToCommentId) {
